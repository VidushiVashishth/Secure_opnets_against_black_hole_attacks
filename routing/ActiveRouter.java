...
public abstract class ActiveRouter extends MessageRouter {
	...
	private double startTime; //Anshuman
	private double elapsedTime; //Anshuman
	private HashMap<DTNHost, Integer> PTfrequency = new HashMap<DTNHost, Integer>(); //Vidushi

	private Message PT; //Anshuman
	...
	protected int startTransfer(Message m, Connection con) {
		...
		if(SimClock.getTime() > 1000) {
            		...
            		DTNHost currentFrom = getHost();
            		DTNHost currentTo = con.getOtherNode(currentFrom);

            		Integer countPTs = 0;

            		if(this.PTfrequency.get(currentTo) != null)
            		countPTs = this.PTfrequency.get(currentTo);

            		Integer countTos = 0;

            		Collection<Message> bufferOfFrom = currentFrom.getMessageCollection();
            		Iterator<Message> it = bufferOfFrom.iterator();
            		while(it.hasNext()) {
                	Message temp = it.next();
                	if(temp.getFrequency().get(currentTo) != null)
                	countTos += temp.getFrequency().get(currentTo);

            	}

            	if(countPTs !=0)
            {
                	if(countTos - countPTs < 2) //Anshuman
                    	return DENIED_TTL;
            }
	...
	protected Message tryAllMessages(Connection con, List<Message> messages) {
		for (Message m : messages) {
			...
			if (retVal == RCV_OK) {
				elapsedTime = (new Date()).getTime() - startTime; //Anshuman
	 ...
	 * accepted a message.
	 */
	protected Connection tryMessagesToConnectionsWithDefense(List<Message> messages,
			List<Connection> connections) { //Anshuman
		for (int i=0, n=connections.size(); i<n; i++) {
			Connection con = connections.get(i);
			startTime = System.currentTimeMillis(); //Anshuman
			Message started = tryAllMessages(con, messages);

			if (started != null && elapsedTime <= 0.99) { //Anshuman
					return con;
			}
			...
			//Replacing else with else if. Might increase delivery probability
			else if(started!= null && elapsedTime >= 0.99) { //created PT. Now in snw we can allow transmission based on frequency and nrof PTs. Anshuman
				DTNHost suspiciousNode = con.getOtherNode(getHost());

				for(Connection connec : getHost().getConnections())
				{
					PT = new Message(getHost(), connec.getOtherNode(getHost()), "PT" + suspiciousNode.getName(), 4 );
					getHost().createNewMessage(PT);
				}

				if(PTfrequency.get(suspiciousNode) != null) //Vidushi
                		{
                    			int v = PTfrequency.get(suspiciousNode);
                    			v++;
                    			PTfrequency.put(suspiciousNode, v);
                		}

				else
		                {
		                    PTfrequency.put(suspiciousNode, 1);
                		}
	...
	protected Connection tryMessagesToConnectionsWithoutDefense(List<Message> messages,
			List<Connection> connections) { //Anshuman
		for (int i=0, n=connections.size(); i<n; i++) {
			Connection con = connections.get(i);
			startTime = System.currentTimeMillis(); //Anshuman
	...
	protected Connection tryAllMessagesToAllConnections(){
		...
		return tryMessagesToConnectionsWithDefense(messages, connections);
	}

	/*protected Connection tryMessageToAllConnections(Message m) {
		List<Connection> connections = getConnections();
