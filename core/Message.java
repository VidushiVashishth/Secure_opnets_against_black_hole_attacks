...
public class Message implements Comparable<Message> {
	...
	private HashMap<DTNHost, Integer> frequency; //Vidushi
	...
	public Message(DTNHost from, DTNHost to, String id, int size) {
		...
		this.appID = null;
		this.frequency = new HashMap<DTNHost, Integer>(); //Vidushi
	...
	public void addNodeOnPath(DTNHost node) {
		...
		/**changes Vidushi**/
		if(this.frequency.get(node) == null)
            	this.frequency.put(node, 1);
        
		else
	        {
            		int v = this.frequency.get(node);
            		v++;
            		this.frequency.put(node, v);
        	}

        	/**changes end Vidushi**/
	}

    /** returns hashmap of frequencies **/ //Vidushi
	public HashMap<DTNHost, Integer> getFrequency()
	{
